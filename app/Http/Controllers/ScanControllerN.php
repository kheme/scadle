...
class ScanControllerN extends Controller{
    public function DoUpload(request $request){
        if($request->ajax()){
            ...
            if ($file->isValid() && $file->getClientOriginalExtension() == $request->ext) {
                ...
                // append uploaded file to list of uploaded files
            ...
            // check if files were uploaded, else display error
     ...
     * @return void
     */
    public function DoScan(request $request, $rand){
        // for error messages
        $error  = null;
        ...
        // parse file with list of uploaded files into an array
        $listFile   = $folder.$rand."_list.txt";
        $handle     = fopen($listFile, "r");
        $buffer     = fread($handle, filesize($listFile));
        $uploaded   = array_filter(explode("\n", $buffer));

        if(!$request == null){
            $request->level = 4;
        }

        // sort file names alphabetically in array
        sort($uploaded);
        
        // check if the user uploaded only one file
        if(count($uploaded) == 1){
            return view('home', ['error' => 'ERROR: Please select at least TWO files to compare']);
        }
        ...
        foreach($uploaded as $file){
            ...
            if(file_exists($file)){
                ...
                // operators
                $operators = array('/>>=/ui', '/>>=/ui', '/<<=/ui', '/>>>/ui', '/%=/ui', '/\^=/ui', '/\|=/ui', '/&=/ui', '/\/=/ui', '/\*=/ui', '/-=/ui', '/\+=/ui', '/>>/ui', '/<</ui', '/\+\+/ui', '/\|\|/ui', '/&&/ui', '/!=/ui', '/>=/ui', '/<=/ui', '/==/ui', '/::/ui', '/->/ui', '/%/ui', '/\^/ui', '/\|/ui', '/&/ui', '/\//ui', '/\*/ui', '/-/ui', '/\+/ui', '/:/ui', '/\?/ui', '/~/ui', '/!/ui', '/</ui', '/>/ui', '/=/ui');
                ...
                */
                ...
                $code = preg_replace('/("|\'|“|”|`).*?("|\'|“|”|`)(*SKIP)(*FAIL)|(-?\d+(?:\.\d+)?)\b/', ' 2 ', $code );     // numbers only: -?\d+(?:\.\d+)?
                ...
                // replace operators
                $code = preg_replace ($operators, ' 4 ', $code);

                // remove curly brackets
                //$code = preg_replace ('/({|})/u', '', $code);

                // replace separators
                $code = preg_replace ('/{|}|\(|\)|\[|\]|,|;|:/u', ' 3 ', $code);

                // replace separators
                //$code = preg_replace ('/[^\w_$\t\s]/u', ' 3 ', $code);
        ...
        return view('home', ['results' => $results, 'rando' => $rand, 'error' => $error]);
    }
}
